...
确保 `dataset.yaml` 中包含 `val` 路径，指向至少 200-300 张有代表性的 CrowdHuman 图片（用于 int8 量化校准的激活值统计）：

```yaml
# dataset.yaml 示例
path: /path/to/crowdhuman
train: images/train
val: images/val          # ← 必须有，用于 int8 校准
nc: 2
names: ['head', 'person']
```

### 步骤 2：导出真 int8 TFLite
...
**`data` 参数是核心**：没有它，ultralytics 无法收集激活值统计，只能做 hybrid quantization（I/O 量化 + 内部 float32），这正是当前模型的问题。
...
将验证通过的 `.tflite` 文件和对应的 `.txt` 标签文件复制到：
